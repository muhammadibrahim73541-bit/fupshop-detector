FupShop Detector
A web-based tool that detects fraudulent online shops in real time by analyzing URLs, domain reputation, and web page content.
Table of Contents
Overview
Features
Tech Stack
...
└── README.md                   # This file
How It Works
URL Input — User submits a URL via the browser extension or Gradio web UI.
Normalization — The URL is cleaned and standardized for consistent analysis.
Multi-Source Check — The system queries VirusTotal, URLhaus, and OpenPhish for known malicious domains.
Domain Analysis — WHOIS lookup checks domain age, registrar, and registration details.
Content Scan — Page content is analyzed for scam indicators:
Typosquatting detection
Suspicious form fields (credit card, password harvesting)
Missing or invalid SSL certificates
Hidden redirects
Fake trust badges
ML Scoring — XGBoost model processes extracted features and assigns a scam probability score (0-100%).
Result Display — User sees a clear risk verdict with a detailed breakdown of all checks performed.
Prerequisites
...
docker run -d \
  ...
  -e URLHAUS_API_KEY=your_key \
  -e OPENPHISH_API_KEY=your_key \
  -v $(pwd)/data:/app/data \
  fupshop
...
The FupShop extension icon will appear in your Chrome toolbar
Click the icon on any webpage to scan the current URL
Extension Features
...
Authors
Muhammad Ibrahim — @muhammadibrahim73541-bit
